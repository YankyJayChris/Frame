...
exports.registerAppIconManager = registerAppIconManager;
exports.createIconCompletionItems = createIconCompletionItems;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const SUPPORTED_FORMATS = [".svg", ".png", ".jpg", ".jpeg"];
const DEFAULT_ICON_PATH = "assets/icons/frame-default.svg";
function registerAppIconManager(context) {
    const disposables = [];
    // Command: Open app icon picker
    disposables.push(vscode.commands.registerCommand("frame.openAppIconPicker", async () => {
    ...
    disposables.push(vscode.commands.registerCommand("frame.validateIcons", async () => {
        const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!workspaceRoot) {
            vscode.window.showErrorMessage("No workspace folder open.");
            return;
    ...
    disposables.push(vscode.commands.registerCommand("frame.previewAppIcon", async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor || editor.document.fileName !== "frame.config.json") {
            vscode.window.showWarningMessage("Open frame.config.json to preview app icon.");
            return;
        }
        const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!workspaceRoot)
        ...
        const icon = icons[0];
        if (icon.format === "default") {
            vscode.window.showInformationMessage(`$(file-media) Using default Frame app icon\n\nLocation: ${DEFAULT_ICON_PATH}\nColor: Lime green (#BCFB70)\nSize: 1080×1080px`);
        }
        else {
...
async function setDefaultIcon(configPath, workspaceRoot) {
    try {
        ...
        if ("icon" in config) {
            ...
            vscode.window.showInformationMessage(`✓ Set to default Frame icon (${DEFAULT_ICON_PATH})`);
        }
        else {
            vscode.window.showInformationMessage(`✓ Already using default Frame icon`);
        }
...
async function selectCustomIcon(configPath, workspaceRoot) {
    try {
        // Find all potential icon files
        const iconFiles = findIconFiles(workspaceRoot);
        if (iconFiles.length === 0) {
            vscode.window.showWarningMessage("No icon files (SVG, PNG, JPEG) found in workspace. Create one in assets/icons/ directory.");
            return;
        }
        ...
        vscode.window.showInformationMessage(`✓ Set custom icon: ${relativePath}\n(${formatFileSize(fileSize)})`);
        // Update active editor if it's frame.config.json
        const activeEditor = vscode.window.activeTextEditor;
        if (activeEditor &&
            activeEditor.document.fileName.endsWith("frame.config.json")) {
...
async function showIconInfo(configPath, workspaceRoot) {
    try {
        ...
        const panel = vscode.window.createWebviewPanel("frameIconInfo", "Frame App Icon Info", vscode.ViewColumn.Beside, {});
        panel.webview.html = `
<!DOCTYPE html>
<html>
<head>
...
function getIconInfo(configPath, workspaceRoot) {
    try {
        const content = fs.readFileSync(configPath, "utf-8");
        const config = JSON.parse(content);
        if (!config.icon) {
...
function findIconFiles(workspaceRoot) {
    const iconFiles = [];
...
function createIconCompletionItems(workspaceRoot) {
    const items = [];
    // Suggest default icon
    const defaultItem = new vscode.CompletionItem("$(symbol-file) Default Icon", vscode.CompletionItemKind.EnumMember);
    defaultItem.detail = "Use Frame default lime green icon";
    defaultItem.insertText = "";
    defaultItem.documentation = new vscode.MarkdownString(`**Default Frame App Icon**\n\n- Color: Lime green (#BCFB70)\n- Size: 1080×1080 px\n- Format: SVG\n- Leave \`icon\` field empty or remove it to use default`);
    ...
    for (const file of customIcons) {
        ...
        item.documentation = new vscode.MarkdownString(`**${fileName}**\n\n- Path: \`${relativePath}\`\n- Size: ${formatFileSize(fs.statSync(file).size)}`);
        items.push(item);
    }
    return items;
}
