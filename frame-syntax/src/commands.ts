...
function getFrameBinary(): string {
  return vscode.workspace.getConfiguration('frame').get<string>('path') || 'frame';
}

function getWorkspaceRoot(): string {
  const configured = vscode.workspace.getConfiguration('frame').get<string>('workspaceRoot');
...
async function scaffoldFile(name: string, template: string, relativeDir: string): Promise<void> {
...
export function registerCommands(context: vscode.ExtensionContext, client: LanguageClient): vscode.Disposable[] {
