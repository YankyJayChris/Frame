...
import { createIconCompletionItems } from "./app-icon-manager";
...
class ColorCompletionProvider implements vscode.InlineCompletionItemProvider {
  async provideInlineCompletionItems(
    ...
    const colorMatch = lineBefore.match(
      /(?:color|background|border_color):\s*"([^"]*)$/,
    );
...
class AppIconCompletionProvider implements vscode.InlineCompletionItemProvider {
  async provideInlineCompletionItems(
    document: vscode.TextDocument,
    position: vscode.Position,
    _context: vscode.InlineCompletionContext,
    ...
    const completionItems = createIconCompletionItems(workspaceRoot);
    const results: vscode.InlineCompletionItem[] = [];
    ...
    for (const item of completionItems) {
      ...
      if (
        itemLabel.toLowerCase().includes(partial.toLowerCase()) ||
        (itemInsertText &&
