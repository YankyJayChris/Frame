...
class IconCompletionProvider implements vscode.InlineCompletionItemProvider {
...
class ColorCompletionProvider implements vscode.InlineCompletionItemProvider {
  async provideInlineCompletionItems(
    document: vscode.TextDocument,
    position: vscode.Position,
    _context: vscode.InlineCompletionContext,
...
class FilePathCompletionProvider implements vscode.InlineCompletionItemProvider {
  async provideInlineCompletionItems(
    document: vscode.TextDocument,
    position: vscode.Position,
    _context: vscode.InlineCompletionContext,
    ...
    let searchPrefix = partial.includes('/') ? partial.substring(partial.lastIndexOf('/') + 1) : partial;
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.endsWith('.fr') && entry.toLowerCase().startsWith(searchPrefix.toLowerCase())) {
...
class RouteCompletionProvider implements vscode.InlineCompletionItemProvider {
  async provideInlineCompletionItems(
    document: vscode.TextDocument,
    position: vscode.Position,
    _context: vscode.InlineCompletionContext,
...
export function registerCompletionProviders(context: vscode.ExtensionContext): vscode.Disposable[] {
  const disposables: vscode.Disposable[] = [];

  disposables.push(
    vscode.languages.registerInlineCompletionItemProvider('frame', new IconCompletionProvider())
  );
  disposables.push(
    vscode.languages.registerInlineCompletionItemProvider('frame', new ColorCompletionProvider())
  );
  disposables.push(
    vscode.languages.registerInlineCompletionItemProvider('frame', new FilePathCompletionProvider())
  );
  disposables.push(
    vscode.languages.registerInlineCompletionItemProvider('frame', new RouteCompletionProvider())
  );
